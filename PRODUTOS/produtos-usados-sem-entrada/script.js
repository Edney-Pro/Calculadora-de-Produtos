...
const TAXA_PADRAO = 0.05; // ALTERADO: Taxa padrão agora é 5% a.m.
...
let telaAtual = 1, produtos = [], garantiaSelecionada = GARANTIAS[1]; // Padrão 12 meses
...
function gerarOpcoesDePagamento() {
    ...
    while (opcoesEncontradas.length < 12) {
...
function adicionarProduto() {
    const nomeInput = document.getElementById('produtoNome'); const valorInput = document.getElementById('produtoValor');
    const nome = nomeInput.value.trim(); const valor = parseFloat(valorInput.value);
    if (!nome || !valor || valor <= 0) { mostrarAlerta('Preencha nome e valor válidos.', 'warning'); return; }
    produtos.push({ id: Date.now(), nome, valor });
    atualizarListaProdutos();
    nomeInput.value = ''; valorInput.value = ''; nomeInput.focus();
}
...
function criarBotoesGarantia() {
    ...
    container.innerHTML = GARANTIAS.map((g, index) => `<div class="col-4"><button type="button" class="btn-garantia" id="garantia-btn-${g.meses}" onclick="selectGarantia(${index})">${g.meses} Meses</button></div>`).join('');
}
...
function atualizarLogicaGarantia() {
    ...
    document.querySelectorAll('.btn-garantia').forEach(b => b.classList.remove('active'));
...
function validarTelaAtual() {
    ...
    switch (telaAtual) {
        ...
        case 5: if (!garantiaSelecionada) { mostrarAlerta('Escolha uma garantia.', 'warning'); return false; } return true;
        default: return true;
    }
}
...
function atualizarResumo() {
    ...
    document.getElementById('pagamentoBlock').innerHTML = `<h6 class="summary-block-title"><i class="bi bi-credit-card me-2"></i>Pagamento</h6><p><strong>Valor Total:</strong> ${formatBRL(valorComGarantia)}<br><strong>Parcelamento:</strong> ${meses}x de <strong>${formatBRL(valorParcela)}</strong></p>`;
...
// ==========================================================
// FUNÇÃO WHATSAPP CORRIGIDA E ADAPTADA
// ==========================================================
function enviarWhatsApp() {
    ...
    const numeroWhatsApp = '5544998408460'; // Seu número com código do país e DDD
...
// --- MODAL DE SIMULAÇÃO DE ATRASO (ATUALIZADO) ---
function abrirModalAtraso() {
    if (!garantiaSelecionada) { mostrarAlerta('Finalize a simulação principal primeiro.', 'warning'); return; }
    const meses = parseInt(document.getElementById('mesesPagamento').value);
...
function calcularSimulacaoAtraso() {
    ...
    // ALTERADO: Multa para 5% e Juros para 0,1% ao dia
    const multa = valorParcela * 0.05; // 5% de multa
    const juros = (valorParcela * 0.001) * diasAtraso; // 0,1% de juros ao dia
