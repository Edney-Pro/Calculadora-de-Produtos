...
// ATUALIZADO: Taxa de juros padrão para 15%
...
// --- INICIALIZAÇÃO e LÓGICA DE CONFIGURAÇÕES (sem alterações) ---
...
// --- LÓGICA DE CÁLCULO e LÓGICA DAS TELAS (sem alterações) ---
...
// --- NAVEGAÇÃO E VALIDAÇÃO (sem alterações) ---
...
function validarTelaAtual() {
    ...
    switch (telaAtual) {
        case 1: if (!document.getElementById('clientName').value.trim() || document.getElementById('clientCPF').value.replace(/\D/g, '').length !== 11) { mostrarAlerta('Nome e CPF são obrigatórios.', 'warning'); return false; } if (!dataPrimeiraParcela) { mostrarAlerta('Escolha o dia do vencimento.', 'warning'); return false; } return true;
        case 2: if (!clientStatus) { mostrarAlerta('Selecione seu status.', 'warning'); return false; } return true;
        case 3: if (clientStatus === 'yes' && !clientSituation) { mostrarAlerta('Selecione sua situação.', 'warning'); return false; } if (clientStatus === 'no' && !sourceInfo) { mostrarAlerta('Selecione como nos conheceu.', 'warning'); return false; } if (sourceInfo === 'indicado' && !indicadoName) { mostrarAlerta('Informe quem indicou.', 'warning'); return false; } return true;
...
function enviarWhatsApp() {
    ...
    // ATUALIZADO: Título da mensagem do WhatsApp
