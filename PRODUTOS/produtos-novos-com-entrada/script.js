...
/* === FUNÇÃO DE NAVEGAÇÃO HOME GLOBAL (CORREÇÃO DE ERRO: "Função de Home não definida") === */
function goHome() {
    // Tenta voltar para o arquivo index.html na raiz do projeto (o mais comum para apps)
    window.location.href = '/index.html'; 

    // Se a linha acima falhar devido à estrutura de pastas aninhadas,
    // tente usar um caminho relativo, dependendo de onde o arquivo está:
...
// --- LÓGICA DE CÁLCULO e LÓGICA DAS TELAS ---
...
// === FUNÇÃO SUBSTITUÍDA: Nova lógica para gerar 6 datas no formato solicitado ===
function gerarOpcoesDePagamento() {
    ...
    diasFixos.forEach(diaFixo => {
        ...
        // Se o dia já passou (ou é hoje) no mês atual, avança para o próximo mês
        if (dataOpcao <= hoje) {
    ...
    // Ordena as datas para exibir em ordem cronológica
    ...
    datasGeradas.forEach(dataOpcao => {
        ...
        // Novo formato de exibição solicitado: Dia XX / XX/MM/AAAA
        const innerHTML = `
        ...
        // Usa 'btn-selection' para aplicar o CSS de otimização de espaço
...
function calcularSimulacaoAtraso() {
    ...
    // O regex garante que o valor seja parseado corretamente (removendo pontos e trocando vírgula por ponto)
